...
class VertexSubset {
  ...
  uint64_t max_el = 0;
  BitArray *ba = nullptr;
  SlidingQueue<int32_t> *queue = nullptr;
  QueueBuffer<int32_t> *queue_array = nullptr;

public:
  [[nodiscard]] bool sparse() const { return is_sparse; }
  [[nodiscard]] bool complete() const { return all; }
  [[nodiscard]] bool has(uint64_t i) const {
    if (all) {
      return true;
    }
    if (is_sparse) {
  ...
  [[nodiscard]] bool has_dense_no_all(uint64_t i) const { return ba->get(i); }
  void has_dense_no_all_prefetch(uint64_t i) const { return ba->prefetch(i); }
  ...
  void insert_dense(uint64_t i) const { return ba->set(i); }
  void insert_sparse(uint64_t i) const {
    queue_array[4 * getWorkerNum()].push_back(i);
  }
  ...
  VertexSubset(bool const *const els, uint64_t len)
      ...
      if (is_sparse) {
        ...
        if (other.queue_array) {
          ...
          for (int i = 0; i < getWorkers(); i++) {
            new (&queue_array[i * 4]) QueueBuffer<int32_t>(
  ...
  [[nodiscard]] VertexSubset convert_to_dense() const {
    ...
    parallel_for(uint32_t i = queue->shared_out_start;
                 i < queue->shared_out_end; i++) {
  ...
  [[nodiscard]] VertexSubset convert_to_sparse() const {
    ...
    vs.max_el = max_el;
    vs.is_sparse = true;
    vs.queue = new SlidingQueue<int32_t>(max_el);
    vs.queue_array = (QueueBuffer<int32_t> *)malloc(
        4 * sizeof(QueueBuffer<int32_t>) * getWorkers());
    vs.ba = nullptr;
    for (int i = 0; i < getWorkers(); i++) {
      new (&vs.queue_array[i * 4]) QueueBuffer<int32_t>(*vs.queue);
    }
