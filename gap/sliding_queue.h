...
public:
  ...
  explicit SlidingQueue(size_t shared_size) {
    ...
    shared_out_end = 0;
    shared_in = 0;
    shared = (T *)memalign(32, shared_size * sizeof(T));
    reset();
  }
  explicit SlidingQueue(const SlidingQueue &other, size_t shared_size) {
    ...
    shared = (T *)memalign(32, shared_size * sizeof(T));
    parallel_for(uint64_t i = 0; i < shared_size; i++) {
...
public:
  ...
  explicit QueueBuffer(SlidingQueue<T> &master, size_t given_size = 16384)
    ...
    in = 0;
    local_queue = (T *)memalign(32, local_size * sizeof(T));
  }

  ~QueueBuffer() { free(local_queue); }
