...
template <typename T, typename SM> struct GET_MIN_DEGREE_F {
  T *match;
  T *maxDegree;
  // vertex* V;
  // PR_F(double* _p_curr, double* _p_next, vertex* _V) :
  SM &G;
  GET_MIN_DEGREE_F(T *_numToEdgesRemove, T *_maxDegree, SM &_G) : 
  ...
  inline bool update(uint32_t s, uint32_t d) { // Update
    ...
    // match[d] == 0 : blue
    // match[s] == 1 : red
    if (match[d] == 0 && match[s] == 1 && G.getDegree(s) < maxDegree[d]){
      // a match
      maxDegree[d] = G.getDegree(s);
    }
    return deletedEdge;
  }
...
template <typename T, typename SM> struct REQUEST_2_F {
  ...
  // PR_F(double* _p_curr, double* _p_next, vertex* _V) :
  SM &G;
  REQUEST_2_F(T *_numToEdgesRemove, T *_maxNumToEdgesRemove, T *_maxDegree, SM &_G) : 
  match(_numToEdgesRemove),
